...
module dual_running_sum #(
	...
	always @(posedge clock) begin
		long_shift_rescale_bis <= long_sum_reg * K >> 3; // We rescale the long term sum by the threshold value and noralize by the delay line length
	...
	assign long_shift_rescale  = long_shift_rescale_bis; // We rescale the long term sum by the threshold value and noralize by the delay line length

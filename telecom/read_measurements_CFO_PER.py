    ...
    for i in range(number_of_txp//largeur):
        for j in range(largeur):
            if count <= number_of_txp:
                ...
                # centre x en 11500
                if (vals[count] in [-11, -12, -13]):
                    axs[i, j].set_xlim(10000, 13000)
                elif (vals[count] in [-19, -18, -17, -16, -15, -14]):
                    axs[i, j].set_xlim(9000, 13000)
                count += 1
        

    plt.tight_layout()
    plt.savefig("plots/CFO_valuesCentered.pdf")
    plt.show()

...
class Chain:
    ...
    def demodulate(self, y: np.array) -> np.array:
        ...
        """
        #r1 = 1/self.osr_rx * np.exp( - 1j * 2 * np.pi * self.freq_dev * (np.arange(self.osr_rx) / self.osr_rx) / self.bit_rate) @ y
        #r0 = 1/self.osr_rx * np.exp( + 1j * 2 * np.pi * self.freq_dev * (np.arange(self.osr_rx) / self.osr_rx) / self.bit_rate) @ y
        #return np.array([1 if np.abs(r1) > np.abs(r0) else -1])  
        raise NotImplementedError
